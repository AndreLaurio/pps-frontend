    ...
    <v-layout justify-center>
      <v-flex xs12 sm12 xl10 md10>
        ...
        <div class="mt-6">
                ...
                <v-dialog v-model="addExamineeDialog.show">
                  ...
                  <v-card>
                    ...
                    <v-card-text>
                      <v-container>
                        <v-alert
                          v-model="addExamineeDialog.success.show"
                          transition="fade-transition"
                          type="success"
                        >
                        ...
                        <v-alert
                          v-model="addExamineeDialog.error.show"
                          transition="fade-transition"
                          type="error"
                        >
                            ...
                            <tr>
                              ...
                              <td class="text-center">
                                <v-btn
                                  class="mx-2 error"
                                  small
                                  @click="addExaminee(ex.item.user_id, ex.item)"
          ...
          <v-container>
            <v-dialog
              v-model="delExamineeDialog.show"
              persistent
              max-width="290"
            >
              <v-card>
                <v-card-title class="pop"
                  ><h3>Remove Examinee</h3></v-card-title
                >
                <v-card-text>{{ delExamineeDialog.message }}</v-card-text>
                <v-card-text
                  >Are you sure to remove the examinee from the
                  exam?</v-card-text
                >
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn
                    color="error"
                    text
                    @click="delExamineeDialog.show = false"
                    >No</v-btn
                  >
                  <v-btn color="primary" text @click="delExaminee()"
                    >Yes, continue</v-btn
                  >
                </v-card-actions>
              </v-card>
...
export default {
  ...
  methods: {
    ...
    delExaminee() {
      this.removeExaminee(
        this.delExamineeDialog.user_id,
        this.delExamineeDialog.item
      );
      this.delExamineeDialog.show = false;
    },
    removeExaminee(user_id, item) {
      axios
        .post("/api/exam/examinee/delete", {
          user_id: user_id,
