                    ...
                    <v-card elevation="15" v-for="(item, item_no) in exam.exam_items" :key="item.item_no" class="pa-4 mb-4 rounded-xl">
                        ...
                        <div v-if="item.question_type_code == 'SCQ'" class="ml-10">
                            <v-radio-group v-model="item.answer" :mandatory="false">
                                <v-radio color="red darken-3" v-for="choice in item.choices" :key="choice.choice_no" :value="choice.choice_no" :label="choice.label" disabled></v-radio>
                            </v-radio-group>
                        </div>
    ...
    methods:{
        loadData(){
            if (this.user_id == 0) {
                ...
                axios.post('/api/exam/examinee/answer/view', {
