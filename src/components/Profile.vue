  ...
  <div class="pop mt-5 ml-12">
    ...
    <v-layout justify-center>
      <v-flex xs12 sm12 md3 xl3>
        <v-avatar size="180px">
          <v-img :src="photo.src" aspect-ratio="1" class="grey lighten-2">
          </v-img>
        </v-avatar>
        <input
          class="mt-5"
          ...
          <v-card-text class="ml-12">
            <v-layout>
              ...
              <v-flex>
                ...
                ></v-text-field>
                Extension Name<v-text-field
                  solo
                  dense
                  v-model="user_details.extension_name"
                ></v-text-field>
                Sex<v-select
                  solo
                  dense
            ...
            <v-layout>
              ...
              <v-flex>
                Password<v-text-field
                  dense
                  solo
                  v-model="user_password.password"
            ...
            <v-layout>
              <v-flex>
                Confirm Password<v-text-field
                  dense
                  solo
                  v-model="user_password.confirm_password"
                  :rules="[rules.required, rules.min]"
                  counter
                  hint="At least 8 characters"
                  :append-icon="
                    confirmation_password ? 'mdi-eye' : 'mdi-eye-off'
                  "
                  :type="confirmation_password ? 'text' : 'password'"
                  @click:append="confirmation_password = !confirmation_password"
                  required
                ></v-text-field>
              </v-flex>
              <v-flex>
  ...
  methods: {
    ...
    changePassword() {
      ...
      if (this.user_password.email != this.user_email) {
        ...
        this.user_password.password == this.user_password.confirm_password
      ) {
        //get the user id
        let user_id = this.user_id;
    ...
    changePhoto() {
      ...
      if (
        this.photo.img["type"] !== "image/jpeg" &&
        this.photo.img["type"] !== "image/png"
      ) {
        this.photo.message = "Please select a photo.";
        return;
      }
      ...
      axios
        .post(`/api/changephoto/${this.user_id}`, formData, config)
        .then((response) => {
